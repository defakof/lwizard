...
void    LWizardNexusApi::setApiKey(const QString& key) { m_apiKey = key.trimmed(); }
bool    LWizardNexusApi::hasApiKey() const             { return !m_apiKey.isEmpty(); }
QString LWizardNexusApi::apiKey()    const             { return m_apiKey; }
...
void LWizardNexusApi::scanModAsync(const QString& modName,
                                   int            nexusId,
                                   const QString& language)
{
  ...
  int queuedCount = 0;
  {
    ...
    if (m_queuePending.contains(modName)) {
      LWizardLog::debug(QStringLiteral("Nexus: auto-scan already queued for '%1'")
                            .arg(modName));
      return;
    }
  ...
  LWizardLog::info(
      QStringLiteral("Nexus: queued auto-scan for '%1' (Nexus #%2, queue depth: %3)")
          .arg(modName).arg(nexusId).arg(queuedCount));

  // Kick the queue on the main thread (this method may be called from any context)
  QMetaObject::invokeMethod(this, [this]() { startNextQueued(); },
                            Qt::QueuedConnection);
}
...
void LWizardNexusApi::startNextQueued()
{
  ...
  LWizardLog::info(
      QStringLiteral("Nexus: starting auto-scan for '%1' (Nexus #%2, lang: %3)")
          .arg(entry.modName).arg(entry.nexusId).arg(entry.language));
                                         ...
                                         const QString& language)
{
  ...
  const QString url = QStringLiteral("https://www.nexusmods.com/")
                      + QLatin1String(k_gameId)
                      + QStringLiteral("/mods/%1").arg(nexusModId);

  LWizardLog::info(QStringLiteral("Nexus: scraping %1 for %2 translations")
                       .arg(nexusModId).arg(nexusLang));
...
void LWizardNexusApi::onModPageReply()
{
  ...
  if (reply->error() != QNetworkReply::NoError || status == 403 || status == 429) {
    const QString msg = reply->error() != QNetworkReply::NoError
                            ...
                            : QStringLiteral("HTTP %1 — Nexus blocked the request. "
                                             "Log in to Nexus first or try again later.").arg(status);
  ...
  if (modIds.isEmpty()) {
    LWizardLog::info(QStringLiteral("Nexus: no %1 translations found for request %2")
                         .arg(nexusLang, reqId));
    finishSearch(reqId, {});
    return;
  }
  ...
  if (!hasApiKey()) {
    ...
    for (int modId : modIds) {
      ...
      f.modId      = modId;
      f.modPageUrl = QStringLiteral("https://www.nexusmods.com/") +
                     QLatin1String(k_gameId) +
                                                    ...
                                                    const QString& nexusLang) const
{
  ...
  LWizardLog::debug(
      QStringLiteral("Nexus HTML parse: translations_ul=%1  flag-%2=%3  html_bytes=%4")
          .arg(hasTranslationsUl).arg(nexusLang).arg(hasFlagClass).arg(html.size()));
  ...
  const QRegularExpression reAnchor(
      QStringLiteral("<a\\b[^>]*\\bflag-") + nexusLang + QStringLiteral("\\b[^>]*>"),
      QRegularExpression::CaseInsensitiveOption);

  // ── Pass 2: extract mod ID from the href on the same tag ─────────────────
  static const QRegularExpression reModId(
      QStringLiteral("href\\s*=\\s*[\"'][^\"']*/mods/(\\d+)"),
      QRegularExpression::CaseInsensitiveOption);
  ...
  while (it.hasNext()) {
    ...
    if (modId > 0 && !seen.contains(modId)) {
      ...
      LWizardLog::debug(QStringLiteral("Nexus: found translation mod %1 in tag: %2")
                            .arg(modId).arg(anchorTag.left(120)));
    }
  }
                                     ...
                                     const QString&    nexusLang)
{
  ...
  for (int modId : modIds) {
    ...
    const QString path =
        QStringLiteral("https://api.nexusmods.com/v1/games/")
        + QLatin1String(k_gameId)
        + QStringLiteral("/mods/")
        + QString::number(modId)
        + QStringLiteral("/files.json?category=main%2Cupdate%2Coptional%2Cold_version%2Cmiscellaneous");
...
void LWizardNexusApi::onFilesReply()
{
...

  {
    ...
    if (reply->error() == QNetworkReply::NoError) {
      ...
      LWizardLog::debug(QStringLiteral("Nexus: got %1 file(s) for mod %2")
                            .arg(files.size()).arg(modId));
    } else {
      LWizardLog::warn(QStringLiteral("Nexus: files API failed for mod %1: %2")
                           .arg(modId).arg(reply->errorString()));
    }
  ...
  if (finished) {
    ...
    std::sort(done.begin(), done.end(),
              [](const NexusTranslationFile& a, const NexusTranslationFile& b) {
